# -*- coding: utf-8 -*-
"""
Created on Wed Sep  1 00:42:16 2021
...
def single_unet(dyn_input):
    conv1 = Conv3D(16, (3, 3, 3), activation='relu', padding='same')(dyn_input)
    ...
    up1 = concatenate([Conv3DTranspose(32, (2, 2, 2), strides=(1, 2, 2), padding='same')(conv3), conv2], axis=4)
    up1 = BatchNormalization()(up1)
    
    conv4 = Conv3D(32, (3, 3, 3), activation='relu', padding='same')(up1)
    conv4 = BatchNormalization()(conv4)
    conv4 = Conv3D(32, (3, 3, 3), activation='relu', padding='same')(conv4)
    conv4 = BatchNormalization()(conv4)
    
    up2 = concatenate([Conv3DTranspose(16, (2, 2, 2), strides=(1, 2, 2), padding='same')(conv4), conv1], axis=4)
    up2 = BatchNormalization()(up2)
    
    conv5 = Conv3D(16, (3, 3, 3), activation='relu', padding='valid')(up2)
    conv5 = BatchNormalization()(conv5)
    conv5 = Conv3D(16, (3, 3, 3), activation='relu', padding='valid')(conv5)
    conv5 = BatchNormalization()(conv5)
    conv5 = Conv3D(16, (3, 3, 3), activation='relu', padding='valid')(conv5)
    conv5 = BatchNormalization()(conv5)
...
def conv_clstm():
    inputs1 = Input((img_depth, img_rows, img_cols, 1),name="dyn1")
    inputs2 = Input((img_depth, img_rows, img_cols, 1),name="dyn2")
    inputs3 = Input((img_depth, img_rows, img_cols, 1),name="dyn3")
    inputs4 = Input((img_depth, img_rows, img_cols, 1),name="dyn4")
    ...
    time_series=concatenate([conv_out_1, conv_out_2,conv_out_3,conv_out_4], axis=1)
...
def train():
    ...
    imgs_train_dyn1=imgs_train_dyn1[:,:,:,:,np.newaxis]
    imgs_train_dyn2=imgs_train_dyn2[:,:,:,:,np.newaxis]
    imgs_train_dyn3=imgs_train_dyn3[:,:,:,:,np.newaxis]
    imgs_train_dyn4=imgs_train_dyn4[:,:,:,:,np.newaxis]
    ...
    imgs_dev_dyn1=imgs_dev_dyn1[:,:,:,:,np.newaxis]
    imgs_dev_dyn2=imgs_dev_dyn2[:,:,:,:,np.newaxis]
    imgs_dev_dyn3=imgs_dev_dyn3[:,:,:,:,np.newaxis]
    imgs_dev_dyn4=imgs_dev_dyn4[:,:,:,:,np.newaxis]
